...
const  GenreFilm: React.FC = () => {
  ...
  const handleSelectItem = (itemKey : any) => {
    handleShow2();
    const itemDetails = genre_film.find((item: { idGenre: any; }) => item.idGenre === itemKey);
    setSelectedItem(itemDetails);
  };

 	const [genre_film, setGenre_film] = useState([]);
  ...
  const handleSaveSubmit = async (event : any) => {
    ...
    try {
      const response = await fetch(url + 'genre_film', {
        method: 'POST',
  ...
  const handleDeleteClick = async (item : any) => {
	...
	const handleInputIdGenreChange = (event :any) => {
		setSelectedItem({ ...selectedItem, idGenre: event.target.value });
	};
	
	

    	useEffect(() => {
		const getGenre_film = async () => {
			try {
				const response = await fetch(url + 'genre_film?page='+ (currentPage - 1));
					if (!response.ok) {
						throw new Error('Network response was not ok');
					};
				const data = await response.json();
				setGenre_film(data.content);
  ...
  return (
	...
	<div className="mb-3"> 
		...
		<td>{item.idGenre}</td>
                  ...
                  <td>
                    <IonButton onClick={() => handleSelectItem(item.idGenre)}>
                      Update
                    </IonButton>
                  </td>
                </tr>
	...
	<div className="mb-3"> 
	 	...
	 	<input className="form-control" type="hidden" name="idGenre" onChange={handleInputIdGenreChange} value={selectedItem ? selectedItem.idGenre:''} />
	</div>
